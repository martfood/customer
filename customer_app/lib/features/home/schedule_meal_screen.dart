...
class ScheduleMealScreen extends StatefulWidget {
  const ScheduleMealScreen({super.key});

  @override
  State<ScheduleMealScreen> createState() => _ScheduleMealScreenState();
}

class _ScheduleMealScreenState extends State<ScheduleMealScreen> {
  ...
  Future<void> _geocodeAddress(String address) async {
    try {
      ...
      debugPrint('ScheduleMealScreen: Error geocoding address: $e');
    }
  }
    ...
    for (var collName in collectionsToListen) {
      final sub = FirebaseFirestore.instance
        ...
        final items = snap.docs.map((doc) {
          ...
          final cat = (item['mealCategory'] ?? '').toString().toLowerCase();
        ...
        debugPrint('Error listening to scheduled meals in $collName: $err');
      });
                ...
                builder: (context, snapshot) {
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            SizedBox(height: 8.h),
                            Text(
                              'There are no bulk or scheduled meals in your delivery area right now.',
                              style: TextStyle(
                                fontSize:
                                    AppTypography.font(AppFontSizes.bodyMedium),
                                color: mutedTextColor,
                  ...
                  return GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final title =
                          (item['name'] ?? 'Scheduled Order Item').toString();
                      final photoUrl =
                          (item['photoUrl'] ?? item['imageUrl'] ?? '')
                              .toString();
                      final price =
                          (item['basePrice'] ?? item['price'] ?? 0).toDouble();
                      final promoPriceVal =
                          (item['promoPrice'] ?? 0).toDouble();
                      final vendorId = (item['vendorId'] ?? '').toString();
                      final vendorInfo = _vendorsMap[vendorId];
                      final profile = vendorInfo?['businessProfile']
                      ...
                      return BulkMealCard(
                        ...
                        imageUrl: photoUrl,
                        price: price > 0
                            ? PriceHelper.applyMarkup(price,
                                item['sourceCollection'] ?? 'resturantPosts')
                            : 0,
                        promoPrice: promoPriceVal > 0
                            ? PriceHelper.applyMarkup(promoPriceVal,
                                item['sourceCollection'] ?? 'resturantPosts')
                            : null,
